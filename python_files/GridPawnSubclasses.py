...
class Predator(GridPawnAgent):
    ...
    def get_best_move(self, prey_location_details):
        ...
        self.__log.info('possible predator moves: {}'.format(possible_moves))
        ...
        self.__log.info('moving to square {}'.format(possible_moves[best_move_index]))
    ...
    def simple_hunt_strategy(self):
        ...
        if(nearest_prey):
            ...
            if prey_location_details:
                ...
                #self.__log.info('could not find a path from {} to nearest prey {}'.format(self,nearest_prey))
                self.random_movement()
        else:
            #move to a random unoccupied square
            #self.__log.info(self.__str__()+'could not find prey')
            self.random_movement()
...
class Prey(GridPawnAgent):
    ...
    def simple_evade_strategy(self):
        self.__log.info('{} implementing simple evade strategy'.format(self))
        nearest_predator = self.find_nearest_predator()
        if(nearest_predator):
            self.__log.info('Nearest Predator {} detected'.format(nearest_predator))
            ...
            if perceived_predator_location_details:
                #make the best move given the location of the nearest predator
                 if self.move(self.get_naive_best_move(perceived_predator_location_details)):
                     self.__log.info('{} moving strategically'.format(self))
                     pass
                 else:
                     self.__log.info('{} moving randomly'.format(self))
                     self.random_movement()
        else:
            self.__log.info('couldnt find nearest predator, moving randomly')
            self.random_movement()
